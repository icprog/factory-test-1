...
unsigned long g_usFileDataIndex     = 0;
unsigned long g_usFileBufferSize    = 0;
...
unsigned char * g_pucFileBuffer     = NULL;
...
signed char ispVMCode(unsigned char a_ucCardType,unsigned char a_ucSlotId);
signed char ispVMDataCode();
long int ispVMDataSize();
void ispVMData( unsigned char * Data );
signed char ispVMShift( signed char Code,unsigned char a_ucCardType,unsigned char a_ucSlotId);
signed char ispVMAmble( signed char Code );
signed char ispVMLoop( unsigned short a_usLoopCount,unsigned char a_ucCardType,unsigned char a_ucSlotId);
...
signed char ispVMLCOUNT( unsigned short a_usCountSize,unsigned char a_ucCardType,unsigned char a_ucSlotId);
void ispVMClocks( unsigned short Clocks,unsigned char a_ucCardType,unsigned char a_ucSlotId);
void ispVMBypass( signed char ScanType, unsigned short Bits,unsigned char a_ucCardType,unsigned char a_ucSlotId);
void ispVMStateMachine( signed char NextState,unsigned char a_ucCardType,unsigned char a_ucSlotId);

void ispVMStart(unsigned char a_ucCardType,unsigned char a_ucSlotId);
void ispVMEnd(unsigned char a_ucCardType,unsigned char a_ucSlotId);
signed char ispVMSend(unsigned short int a_usiDataSize,unsigned char a_ucCardType,unsigned char a_ucSlotId);
signed char ispVMRead(unsigned short int a_usiDataSize,unsigned char a_ucCardType,unsigned char a_ucSlotId);
signed char ispVMReadandSave(unsigned short int a_usiDataSize,unsigned char a_ucCardType,unsigned char a_ucSlotId);
...
extern int readPort(unsigned char a_ucCardType,unsigned char a_ucSlotId);
extern int writePort( unsigned char pins, unsigned char value,unsigned char a_ucCardType,unsigned char a_ucSlotId);
extern void sclock(unsigned char a_ucCardType,unsigned char a_ucSlotId);
...
extern const unsigned char g_ucPinENABLE;/*not suppot */
	...
	while ( ( cOpcode = GetByte() ) >= 0 ) {
		...
		case STATE:
			...
			***************************************************************/
			...
			{
				ispVMStateMachine( DRCAPTURE,ucCardType,ucSlotId);
			}

			ispVMStateMachine( ucState,ucCardType,ucSlotId);
		...
		case XSDR:
			...
			***************************************************************/
			...
			cRetCode = ispVMShift( cOpcode,ucCardType,ucSlotId);
			if ( cRetCode != 0 ) {
				return ( cRetCode );
			}
			break;
		...
		case TCK:
			...
			***************************************************************/
			...
			ispVMClocks( usToggle,ucCardType,ucSlotId);
		...
		case REPEAT:
			...
			***************************************************************/
			...
			cRetCode = ispVMLoop( ( unsigned short ) iRepeatSize,ucCardType,ucSlotId);
			if ( cRetCode != 0 ) {
				return ( cRetCode );
			}
			break;
		...
		case LCOUNT:
			...
			***************************************************************/
			...
			cRetCode = ispVMLCOUNT( ( unsigned short ) ispVMDataSize(),ucCardType,ucSlotId);
			if ( cRetCode != 0 ) {
		...
		case HEADER:
			...
			***************************************************************/
			...
			break;
		case ispEN:
            printf("\n ispEN not support OPCODE 0x%.2x\n",cOpcode);
			break;	
		case TRST:
            printf("\n TRST not support OPCODE 0x%.2x\n",cOpcode);
			break;	                
		default:
...
signed char ispVMShift( signed char a_cCode,unsigned char a_ucCardType,unsigned char a_ucSlotId)
{
	...
	case SIR:
		...
		if ( g_usFlowControl & CASCADE ) {
			ispVMStateMachine( SHIFTIR ,ucCardType,ucSlotId);
		}
		else {
			ispVMStateMachine( IRPAUSE,ucCardType,ucSlotId);
			ispVMStateMachine( SHIFTIR,ucCardType,ucSlotId);
			if ( g_usHeadIR > 0 ){ 
				ispVMBypass( HIR, g_usHeadIR ,ucCardType,ucSlotId);
				sclock(ucCardType,ucSlotId);
			}
		}
		break;
	case XSDR:  
		...
		if ( g_cCurrentJTAGState != SHIFTDR ) {
			...
			if ( g_usFlowControl & CASCADE ) {
				if ( g_cCurrentJTAGState == DRPAUSE ) {
					ispVMStateMachine( SHIFTDR,ucCardType,ucSlotId);
					...
					if ( g_usHeadDR > 0 ) {
						ispVMBypass( HDR, g_usHeadDR,ucCardType,ucSlotId);
						sclock(ucCardType,ucSlotId);
					}
				}
				else {
				...
				ispVMStateMachine( DRPAUSE,ucCardType,ucSlotId);
				ispVMStateMachine( SHIFTDR,ucCardType,ucSlotId);
				if ( g_usHeadDR > 0 ) {
					ispVMBypass( HDR, g_usHeadDR,ucCardType,ucSlotId);
					sclock(ucCardType,ucSlotId);
	...
	if ( g_usDataType & TDO_DATA || g_usDataType & DMASK_DATA ) {
		if(g_usDataType & DMASK_DATA){
			cRetCode = ispVMReadandSave( g_usiDataSize,ucCardType,ucSlotId);
			if(!cRetCode){
				if ( g_usTailDR > 0 ) {
					sclock(ucCardType,ucSlotId);
					ispVMBypass( TDR, g_usTailDR,ucCardType,ucSlotId); 
				}
				ispVMStateMachine( DRPAUSE,ucCardType,ucSlotId);
				ispVMStateMachine( SHIFTDR,ucCardType,ucSlotId);
				if( g_usHeadDR > 0 ){
				...
				cRetCode = ispVMSend( g_usiDataSize,ucCardType,ucSlotId);
			}
		}
		else{
			cRetCode = ispVMRead( g_usiDataSize,ucCardType,ucSlotId);
			if ( cRetCode == -1 && g_cVendor == XILINX ) {
				for( iReadLoop = 0; iReadLoop < 30; iReadLoop++ ){
					...
					if( !cRetCode ) {
						...
						ispVMStateMachine( DRPAUSE,ucCardType,ucSlotId); /*Always DRPAUSE*/
						/*Bypass other devices when appropriate*/
						ispVMBypass( TDR, g_usTailDR,ucCardType,ucSlotId);
						ispVMStateMachine( g_ucEndDR,ucCardType,ucSlotId);
						ispVMStateMachine( IDLE,ucCardType,ucSlotId);
						ispVMDelay( 1000 );
					}
				}
			}
	...
	case SIR:
		...
		if ( !( g_usFlowControl & CASCADE ) ) {
			if ( g_usTailIR > 0 ) {
				sclock(ucCardType,ucSlotId);
				ispVMBypass( TIR, g_usTailIR,ucCardType,ucSlotId);
			}
			ispVMStateMachine( g_ucEndIR,ucCardType,ucSlotId);
		}
		break;
    case XSDR:
    case SDR: 
...
void ispVMBypass( signed char ScanType, unsigned short Bits ,unsigned char a_ucCardType,unsigned char a_ucSlotId)
{
	...
	for ( iIndex = 0; iIndex < Bits - 1; iIndex++ ) {
		...
		writePort( g_ucPinTDI, cBitState,ucCardType,ucSlotId);
		sclock(ucCardType,ucSlotId);
	}   
...
void ispVMStateMachine( signed char cNextJTAGState,unsigned char a_ucCardType,unsigned char a_ucSlotId)
{
	...
	for ( cPathIndex = 0; cPathIndex < g_JTAGTransistions[ cStateIndex ].Pulses; cPathIndex++ ) {
		if ( ( g_JTAGTransistions[ cStateIndex ].Pattern << cPathIndex ) & 0x80 ) {
			writePort( g_ucPinTMS, ( unsigned char ) 0x01,ucCardType,ucSlotId);
		}
		else {
			writePort( g_ucPinTMS, ( unsigned char ) 0x00,ucCardType,ucSlotId);
		}
		sclock(ucCardType,ucSlotId);
	}
	
	writePort( g_ucPinTDI, 0x00,ucCardType,ucSlotId);
	writePort( g_ucPinTMS, 0x00,ucCardType,ucSlotId);
}
...
void ispVMStart(unsigned char a_ucCardType,unsigned char a_ucSlotId)
{
	...
	ispVMStateMachine( RESET,ucCardType,ucSlotId);    /*step devices to RESET state*/
}
...
signed char ispVMSend( unsigned short a_usiDataSize, unsigned char a_ucCardType,unsigned char a_ucSlotId)
{
	/*09/11/07 NN added local variables initialization*/
	unsigned short iIndex       = 0;
...
signed char ispVMRead( unsigned short a_usiDataSize,unsigned char a_ucCardType,unsigned char a_ucSlotId)
{
	/*09/11/07 NN added local variables initialization*/
	...
	*****************************************************************************/
	...
	for ( usDataSizeIndex = 0; usDataSizeIndex < a_usiDataSize; usDataSizeIndex++ ) { 
        ...
        if(readPort(ucCardType,ucSlotId)!=ERROR)
        {
          cCurBit = readPort(ucCardType,ucSlotId);
        }
        else
        {
          printf("%s :%d readPort error ,value %d\r\n",__FUNCTION__,__LINE__,cCurBit);
        }
		...
		*****************************************************************************/
		...
		writePort( g_ucPinTDI, ( unsigned char ) ( ( ( cInDataByte << cByteIndex ) & 0x80 ) ? 0x01 : 0x00 ),ucCardType,ucSlotId);
	...
	***************************************************************/
	...
	for ( usDataSizeIndex = 0; usDataSizeIndex < a_usiDataSize; usDataSizeIndex++ ) {
		...
		***************************************************************/
		...
		usOutBitIndex++;
		writePort( g_ucPinTDI, cDataByte,ucCardType,ucSlotId);
		if ( usDataSizeIndex < usLastBitIndex ) {
			sclock(ucCardType,ucSlotId);
		}
